...
\begin{lemma}[Irreducible characters are positive definite]
...
\paragraph{Statement.}
\emph{Intuition.} Gauge-fix on a tree so only finitely many chords remain; the Wilson energy is uniformly strictly convex along chords on fixed regions, giving a local log–Sobolev inequality. A Lipschitz bound for the local action then yields subgaussian Laplace tails (Herbst), giving uniform exponential integrability.
Fix a bounded physical region $R\subset\mathbb{R}^4$ and let $\mathcal{P}_R$ be the set of plaquettes in $R$ at spacing $a$. With $\phi(U):=1-\tfrac{1}{N}\mathrm{Re\,Tr}\,U\in[0,2]$ and $S_R(U):=\sum_{p\in\mathcal{P}_R}\phi(U_p)$, there exist $\eta_R>0$ and $C_R<\infty$, depending only on $R$ and $N$, such that for all $(L,a)$ in the scaling window and any boundary configuration outside $R$,
\[
...
\begin{itemize}
  ...
  \item \textbf{OS2 (reflection positivity).} TeX: Sec.~\ref{sec:lattice-setup} and ``Reflection positivity and transfer operator''; OS2 preserved under limits in Appendix C1b. Lean: `YM.OSWilson.wilson_reflected_gram_psd`, `YM.OSWilson.os2_reflection_positivity_limit`.
...
\text{Gap bounds:} \quad & c_{\rm cut} := -\frac{1}{a}\log(1-\theta_* e^{-\lambda_1 t_0}) > 0 && \text{per-tick rate}\\
& \gamma_{\rm cut} := 8 c_{\rm cut} && \text{$\beta$-indep. floor}\\
...
\section{Appendix: $\varepsilon$–uniform cluster expansion along a scaling trajectory (C3)}
...
\section{Appendix: $\varepsilon$–uniform spectral gap from cross–cut couplings (C4)}
